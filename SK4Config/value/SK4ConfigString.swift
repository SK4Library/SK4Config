...
public class SK4ConfigString: SK4ConfigGenerics<String> {
	...
	/// 初期化
	override public init(title: String, identifier: String? = nil, value: String) {
		super.init(title: title, identifier: identifier, value: value)
	}

	/// 文字列の最大長を指定して初期化　※編集を有効にする
	public convenience init(title: String, identifier: String? = nil, value: String, maxLength: Int) {
		self.init(title: title, identifier: identifier, value: value)
